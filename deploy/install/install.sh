...
if [ "$BEEZA_LICENSE_MODE" = "enforce" ]; then
  grep -q '^BEEZA_LICENSE_PUBLIC_KEY=.' "$ENV_FILE" || fail "license public key is required in enforce mode"
  grep -q '^BEEZA_LICENSE_TOKEN=.' "$ENV_FILE" || fail "license token is required in enforce mode"
fi
...
payload = {
    ...
    "metadata": {
        ...
        "release_channel": os.environ.get("BEEZA_RELEASE_CHANNEL", "pilot"),
    },
}
...
log "BeezaOffice ${BEEZA_APP_VERSION} is ready"
log "Install directory: $INSTALL_DIR"
log "Endpoint: ${BEEZA_PUBLIC_URL:-http://127.0.0.1:${APP_PORT:-8080}}"
